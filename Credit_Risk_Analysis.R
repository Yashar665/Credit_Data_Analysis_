...
data <- fread("credit.csv")
setnames(data,"creditability","target")
data <- data %>% mutate(target = ifelse(as.numeric(as.factor(target))>1,1,0))
# data$target <- as.numeric(as.factor(data$target))
...
dt_list <- split_df(data.iv, "target", ratio = 0.8, seed=123)
train <- dt_list$train 
test <- dt_list$test


# woe binning ---
bins <- data.iv %>% woebin("target")
